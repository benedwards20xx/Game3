// major issue: can pop in new tile even when a move shouldn't be possible.
//  This could be a problem with the "available selections", or there should
//  perhaps be a "can move be made" function. also a check game over function
...
function tile(value, x_pos, y_pos, x, y, color) {
    ...
    // this.value_text_color = this.get_value_text_color();
...
function game_over() {
    ...
    restart_button_shape.graphics.setStrokeStyle(2,"square").beginStroke(BLACK_COLOR).beginFill(WHITE_COLOR)
        .drawRoundRect(restart_button_shape.x,restart_button_shape.y,RECT_WIDTH*2,RECT_HEIGHT/2,20);
...
function setup_start_screen() {
    ...
    start_button_shape.graphics.setStrokeStyle(2,"square").beginStroke(BLACK_COLOR).beginFill(WHITE_COLOR)
        .drawRoundRect(start_button_shape.x,start_button_shape.y,RECT_WIDTH*2,RECT_HEIGHT/2,20);
...
function draw_next_tile() {
    ...
    next_tile.graphics.setStrokeStyle(2,"round").beginStroke(GREY_COLOR).beginFill(num_colors_list[next_val-1])
        .drawRoundRect(next_tile.x,next_tile.y,RECT_WIDTH/2,RECT_HEIGHT/2,20);
